...
class AuthServiceClient(client: HttpClient, baseUrl: String) :
        ...
        private suspend fun getAdminToken(): String {
                ...
                if (response.status.isSuccess()) {
                        // Parse the response manually to avoid serialization issues
                        val rawBody = response.bodyAsText()
                        logger.debug { "Raw admin token response: $rawBody" }

                        try {
                                val jsonElement = jsonParser.parseToJsonElement(rawBody)
                                ...
                                val expiresIn =
                                        jsonObject["expires_in"]?.jsonPrimitive?.content
                                                ?.toIntOrNull()
                                                ?: 60
                                ...
                                logger.error(e) {
                                        "Failed to parse admin token response: ${e.message}"
                                }
                                throw e
                        }
                } else {
        ...
        private fun trackToken(userId: String, tokenResponse: TokenResponse) {
                ...
                tokenResponse.accessToken?.let { _ ->
        ...
        suspend fun login(username: String, password: String): TokenResponse {
                ...
                try {
                        ...
                        if (response.status.isSuccess()) {
                                // Parse the response manually to avoid serialization issues
                                val tokenResponse = parseTokenResponse(response)
        ...
        /** Helper function to parse token responses */
        private suspend fun parseTokenResponse(response: HttpResponse): TokenResponse {
                val rawBody = response.bodyAsText()
                logger.debug { "Raw token response: $rawBody" }

                try {
                        val jsonObject = jsonParser.parseToJsonElement(rawBody).jsonObject
                        ...
                        return TokenResponse(
                                ...
                                expiresIn =
                                        jsonObject["expires_in"]?.jsonPrimitive?.content
                                                ...
                                                ?: 0,
                                refreshExpiresIn =
                                        jsonObject["refresh_expires_in"]?.jsonPrimitive?.content
                                                ?.toIntOrNull()
                                                ?: 0,
                                refreshToken = jsonObject["refresh_token"]?.jsonPrimitive?.content,
                                tokenType = jsonObject["token_type"]?.jsonPrimitive?.content,
                                notBeforePolicy =
                                        jsonObject["not-before-policy"]?.jsonPrimitive?.content
                                                ?.toIntOrNull()
                                                ?: 0,
                                sessionState = jsonObject["session_state"]?.jsonPrimitive?.content,
                                scope = jsonObject["scope"]?.jsonPrimitive?.content
                        )
                } catch (e: Exception) {
                        logger.error(e) { "Failed to parse token response: ${e.message}" }
                        throw RuntimeException("Failed to parse token response: ${e.message}", e)
                }
        }
        ...
        suspend fun validateToken(token: String): TokenInfo {
                ...
                try {
                        ...
                        if (response.status.isSuccess()) {
                                ...
                                try {
                                        val jsonObject =
                                                jsonParser.parseToJsonElement(rawBody).jsonObject

                                        return TokenInfo(
                                                active =
                                                        jsonObject["active"]?.jsonPrimitive?.content
                                                                ?.toBoolean()
                                                                ?: false,
                                                exp =
                                                        jsonObject["exp"]?.jsonPrimitive?.content
                                                                ?.toLongOrNull()
                                                                ?: 0,
                                                iat =
                                                        jsonObject["iat"]?.jsonPrimitive?.content
                                                                ?.toLongOrNull()
                                                                ?: 0,
                                                ...
                                                sub = jsonObject["sub"]?.jsonPrimitive?.content
                                                                ?: "",
                                                typ = jsonObject["typ"]?.jsonPrimitive?.content
                                                                ?: "",
                                                azp = jsonObject["azp"]?.jsonPrimitive?.content
                                                                ?: "",
                                                sessionState =
                                                        jsonObject["session_state"]
                                                                ?.jsonPrimitive
                                                                ?.content
                                                                ?: "",
                                                ...
                                                scope = jsonObject["scope"]?.jsonPrimitive?.content
                                                                ?: "",
                                                sid = jsonObject["sid"]?.jsonPrimitive?.content
                                                                ?: "",
                                                email = jsonObject["email"]?.jsonPrimitive?.content,
                                                name = jsonObject["name"]?.jsonPrimitive?.content,
                                                preferredUsername =
                                                        jsonObject["preferred_username"]
                                                                ?.jsonPrimitive
                                                                ?.content
                                        )
                                } catch (e: Exception) {
                                        logger.error(e) {
                                                "Failed to parse token info: ${e.message}"
                                        }
                                        throw e
                                }
                        } else {
